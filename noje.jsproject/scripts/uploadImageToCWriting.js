// scripts/uploadImageToCWriting.js
// Resmi GridFS'e yükleyip "C harfi nasıl yazılır" sorusuna ekler
...
async function uploadImageToCWriting(imagePath) {
    try {
        ...
        return new Promise((resolve, reject) => {
            readStream
                ...
                .on('finish', async () => {
                    ...
                    // "C harfi nasıl yazılır" sorusunu bul
                    const question = await MiniQuestion.findOne({
                        $or: [
                            { 'data.questionText': { $regex: /C harfi nasıl yazılır/i } },
                            { 'data.questionText': { $regex: /C harfı nasıl yazılır/i } },
                            { title: { $regex: /C harfi nasıl yazılır/i } }
                        ]
                    });

                    if (!question) {
                        console.log('\n❌ "C harfi nasıl yazılır" sorusu bulunamadı');
                        console.log('💡 Tüm soruları kontrol ediliyor...\n');
                        
                        // Tüm soruları listele
                        const allQuestions = await MiniQuestion.find({
                            'data.questionText': { $regex: /C/i }
                        }).limit(10).lean();
                        
                        if (allQuestions.length > 0) {
                            console.log(`📋 C içeren sorular (${allQuestions.length} adet):\n`);
                            allQuestions.forEach((q, index) => {
                                console.log(`${index + 1}. ${q.data?.questionText || q.title || 'Başlıksız'} (ID: ${q._id})`);
                            });
                        } else {
                            console.log('❌ C içeren soru bulunamadı');
                        }
                    ...
                    console.log(`\n✅ Resim "C harfi nasıl yazılır" sorusuna eklendi!`);
...
uploadImageToCWriting(imagePath)
    .then(() => {
        console.log('\n✅ İşlem tamamlandı!');
        process.exit(0);
    })
